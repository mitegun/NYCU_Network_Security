...
class LogisticRegression:
    def __init__(self, learning_rate: float = 1e-3, num_iterations: int = 100):
...
class FLD:
    ...
    def fit(
        ...
        self.std = np.std(inputs, axis=0)
        normalized_inputs = (inputs - self.mean) / self.std
        class_0 = normalized_inputs[targets == 0]
        class_1 = normalized_inputs[targets == 1]
    ...
    def predict(
        ...
        normalized_inputs = (inputs - self.mean) / self.std
        projected_inputs = normalized_inputs @ self.w
    ...
    def plot_projection(self, x_test):
        # Plot the projection line and classified points
        fig, ax = plt.subplots()
        y_pred = self.predict(x_test)
        unique_labels = np.unique(y_pred)
        self.intercept = self.m1[1] - (self.slope * self.m1[0])
        colors = ["blue", "red"]  # Example colors for two classes
        for label, color in zip(unique_labels, colors):
            class_points = x_test[y_pred == label]
            ax.scatter(
                class_points[:, 0], class_points[:, 1], c=color, label=f"Class {label}"
            )
        x_vals = np.array(ax.get_xlim())
        y_vals = self.slope * x_vals + self.intercept
        ax.plot(x_vals, y_vals, "--k", label="Projection Line")
        ax.legend()
        ax.set_title(
            "Projection Line: Slope={}, Intercept={}".format(self.slope, self.intercept)
        )
        plt.show()
